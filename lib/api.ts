...
export class ApiClient {
  ...
  static async checkBackendHealth(): Promise<boolean> {
    try {
      const response = await fetch(`${process.env.BACKEND_URL || 'https://void.fusionfocus.icu'}/health`, {
        method: 'GET',
      });
      return response.ok;
    } catch (error) {
  ...
  static async signup(request: SignUpRequest): Promise<AuthResponse> {
    const response = await fetch(`${process.env.BACKEND_URL || 'https://void.fusionfocus.icu'}/api/v1/auth/signup`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  static async login(request: LoginRequest): Promise<AuthResponse> {
    const response = await fetch(`${process.env.BACKEND_URL || 'https://void.fusionfocus.icu'}/api/v1/auth/login`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  static async getCurrentUser(token: string): Promise<UserInfo> {
    const response = await fetch(`${process.env.BACKEND_URL || 'https://void.fusionfocus.icu'}/api/v1/auth/me`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
  ...
  static async logout(token: string): Promise<{ success: boolean; message: string }> {
    const response = await fetch(`${process.env.BACKEND_URL || 'https://void.fusionfocus.icu'}/api/v1/auth/logout`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
